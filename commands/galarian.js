...
module.exports.run = async (client, message, args, prefix) => {
    ...
    var arg = message.content.slice(`${prefix}galarian`.length).split(' ');
    ...
    if (pokemonName === 'random') {
        ...
        // number
        con.query(`SELECT * FROM pokemon WHERE galarian = ?`, [1], function (err, rows) {
        ...
        con.query(`SELECT * FROM pokemon WHERE name = ? AND galarian = ?`, [pokemonName, 1], function (err, rows) {
            // if there is a error, throw it my way
            if (err) {
                throw err;
            }
            ...
            if (rows.length > 0) {
                ...
                // do a quick query, select everything from the table named pokemon where the id is the same as the
                // number
                con.query(`SELECT * FROM pokemon WHERE dex = ? AND galarian = ?`, [pokemonName, 1], function (err, rows) {
                    // if there is a error, throw it my way
                    if (err) {
                        throw err;
                    }
    ...
    function lookupName(rows) {
        ...
        var shinyImageLink = 'https://shinybot.dev/public/sprites/shiny/' + rows.dex + '_galarian.gif';
